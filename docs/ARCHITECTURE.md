...
- `clearQuests(_:workoutID:)` records quest ids on `PlayerProgress.questCompletions` (Optional, keyed by `QuestID.rawValue`) and pays what that clears: NEW GAME (PRESS START, SAY IT OUT LOUD, FINISH) pays +250 once when its last step lands, each STAGE 2 quest (SHOW UP TONIGHT, RITUAL, BEAT YOURSELF, FOUR IN A WEEK) pays +250 once; STAGE 2 listens only after NEW GAME is cleared. Awards join `lastReward` when they belong to the same workout, like a PR bonus. `backfillQuests` marks what an existing player already did with no XP, once (`questsBackfilledAt`). The catalog and the pure evaluator are `Shared/RUXP/Quests.swift`; `RUXP/Quests/QuestService.swift` turns the hooks (`onWorkoutStarted`, `onMomentAdded`, `onRewardChanged`) into ids and stays inactive until `activate()` so the upgrade rebuild never pays quest XP. A Live Ops `multiplier` may name `"reason": "quest"`.
...
### CrewService (`RUXP/Crew/`)
"Who is expecting me?" Your crew is your Game Center friends who lift. `GameCenterService.loadCrew(season:)` reads four boards with the friends scope: the season board for identity, `crew_week` (recurring weekly; score = rewarded workouts this ISO week, submitted after every rewarded completion) for this week, its `loadPreviousOccurrence()` for last week, and the active boards for lifting-now. A member is in the crew when they trained this week or last. `CrewState.from` is the pure summary (names, counts, size, in, met). `CrewService` polls every 3 minutes in the foreground, persists `Documents/crew.json`, and settles CREW WEEK: when every active member (≥ 2 others) and the local player have a workout this week, `ProgressionService.rewardCrewWeek(weekKey:)` pays +250 once per ISO week (`PlayerProgress.crewWeekBonusWeeks`, Optional), joining the open reward summary when it is from the same week; on the first read after a week rolls, last week is settled from the previous occurrence. Crews are ego-centric (my friends, not a shared roster): honest, serverless, and the reason named crews are a later step. The rule for every surface: show who is in, never who is out.
...
### SessionObjectiveProviding (`RUXP/Live/LiveObjective.swift`)
"How much have we moved together tonight?" `GameCenterSessionObjective` reads the daily recurring board `session_volume` (24 h from 06:00 ET, so every US evening session and all of Sunday sit in one window). Each player submits their own cumulative lb for the window (best-score board, so always the sum, never a delta); the reader pages up to 200 global entries every 90 s, sums the scores for the total, takes `totalPlayerCount` as contributors, and keeps the top ten aliases for the floor. A parse that lands after the window rolled keeps the contribution local rather than crediting the next night. The contribution path: `WorkoutProcessor.onWorkoutParsed` → `LiveSessionService.recordContribution` (replace per workout, so re-parses never double count; recorded even with Game Center off) → `objectiveSubmit`. While a session is live, `noteObjectiveUpdated` keeps the night's total, contributors, and peak lifters on the participation, which is what the ticket shows.

### LiveActivityProviding (`RUXP/Live/LiveActivity.swift`)
The floor. `ObservedLiveActivity` never polls; it is a composer over hooks that already fire (`GameCenterLivePresence.onSnapshotChanged`, `CrewService.onSnapshotChanged`, `GameCenterSessionObjective.onStateChanged`, `LiveRoomService.onReaction`, `LiveSessionService.onContributionRecorded`), chained in `RUXPApp.init`. Lines: people started or finished (presence deltas), a crew member lifting now, a player's volume tonight by alias, milestones every 250K LB, reactions, your own join and contribution. Bounded to 30, oldest first. `LiveWorldSimulator` (`#if DEBUG`, `-RUXPLiveDemo`) is the only other implementation and stands behind all three live protocols at once; it is never the release source.
...
### Community (`Shared/RUXP/Community.swift`, `Shared/RUXP/CommunityMoment.swift`, `RUXP/Community/`)
RUXP is the game; Discord is where the squad hangs out between sessions. RUXP keeps only the social features a live workout needs; persistent conversation lives on Discord. Game Center stays the scorekeeper and identity (counts, objective sums, crew boards, ranks, the watch); Discord is never the login.

- **Directory.** `CommunityDirectory` maps events to Discord channels: `byEventID` (a hosted one-off, "GOLD'S GYM VENICE presents") beats `byKind` (`fridayNight`, `sundayReset`, `nightly`) beats `home`. It also carries `relayURL` and `linkOrigin`. `CommunityService` is `LiveOpsService` for `docs/community.json`: bundled (empty) → cached `Documents/community.json` → validated remote, hourly on foreground, version floor, `-RUXPCommunity off|<path>`. `LiveEventProviding.community(for:)` reads `CommunityCatalog.current`; a backend returns the same shape. With no channel for an event the lobby shows no card and nothing posts.
- **Lobby.** `CommunityCard` under START WORKOUT: the channel label, an optional PRESENTED BY line, OPEN LIVE CHAT (`DiscordLinks`: `discord://discord.com/channels/g/c`, then the web URL), an invite line when the directory has one. No Discord branding; theme tokens only.
- **Moments.** A `CommunityMoment` is a gameplay event carved out of private workout data on purpose: never a set, no field for body or health data. `MomentPolicy` is the whole list: player moments (join, PR, level-up, session complete) leave the phone only when Settings › Community › "Share my moments to Discord" is on, and then carry the Game Center alias; aggregate moments (lifters past 10/25/50/100/250/500/1000, volume every 250K LB) carry no identity and post whenever the event has a channel. `CommunityMomentComposer` is chained in `RUXPApp.init` next to the floor (`onJoined`, `presenceHook`, `objectiveHook`, `WorkoutProcessor.onPersonalRecords`, `WorkoutManager.onRewardChanged`), dedups by id, and hands survivors to `CommunityPublishing`. `RelayCommunityPublisher` POSTs JSON to the relay, fire and forget; with no `relayURL` the moment is logged and kept. `sessionLive` and `eventComplete` are reserved for the relay's own schedule.
- **Relay.** `Scripts/discord-relay/worker.js` (Cloudflare Worker) is the only place a Discord token or webhook lives. It validates the shape, dedups on `moment.id` in KV (every phone can report the same milestone; Discord sees it once), renders one embed per kind with a `JOIN WORKOUT` link button, and posts via bot (per-event channel) or webhook. Posts are unauthenticated beyond shape checks and a rate limit; the upgrade is a Game Center identity signature verified at the relay.
- **Links.** `AppRoute` parses `ruxp://join/live`, `ruxp://join/<eventID>` (`ruxp://event/...` alias), and the same paths on `https://ruxp.app`; `/squad`, `/player`, `/challenge`, `/discord/callback` are reserved. `AppRouter` holds one pending route (`.onOpenURL` at the root, `-RUXPOpenURL` in DEBUG); `MainTabView` switches to Home; `HomeView` resolves (`event(id:)`, else the featured event), joins only a live occurrence (joining posts presence), opens the lobby with the existing dismiss-then-start handoff, and waits for a pending season recap or an active workout instead of stacking covers. `AppLinks.join(eventID:)` is what goes into Discord messages and the lobby's share text. Discord link buttons need https, so the join link is a Universal Link: `docs/.well-known/apple-app-site-association` + `applinks:ruxp.app`, with `docs/404.html` as the bridge for a browser that does not hand off (Discord's in-app browser loads the link as an initial page, which does not trigger a Universal Link), offering `ruxp://join/...` and the App Store or TestFlight link from `config.js`.
- **Not built, designed.** Discord account linking (Settings › Connected Accounts: `ASWebAuthenticationSession`, PKCE with the app's Public Client flag, `identify` scope, Keychain, revoke on disconnect; then moments can carry the Discord handle and a guild role can become a Squad roster). Bot slash commands and scheduled LIVE NOW posts in the same Worker. Squads (`ruxp://squad/<id>` is reserved). Presence: Discord's Social SDK is C++ only on iOS and requires account linking; when it comes, it sits behind an opt-in `PresencePublishing` protocol with a coarse payload (`Lifting · FRIDAY NIGHT · 42 min`), never a set or a health number.
...
Squads (a Discord guild role is the shared roster Game Center cannot express; see Community), gym profiles, gym-hosted events (the directory's `byEventID` + `host` is the seam), brand rewards, a rank line on Home, per-lift and total-volume leaderboards (they depend on the OpenAI parse), remote gym experiences, push notifications. Each plugs into one of the protocols above or into `PlayerProgress`. None of them are speculatively wired.
